...
synthesis (HumanEval, 164 problems).  On GSM8K, few-shot CoT raised
accuracy substantially for the 3B model, a gain that rivals the
effect of scaling the model from 1B to 3B.  An algorithmic effect
ratio near one shows that most of the
benefit survives rewording of the prompt.  Temperature was found to
interact with sampling-based techniques: self-consistency requires
nonzero temperature and peaks near 0.7, while single-sample accuracy
declines monotonically as temperature rises.  We conclude with an
...
16~GB unified memory.  The four implemented configurations span all
...
engine on a Metal backend.  Small open-weight models are required
...
\textbf{Hardware.}  All experiments run locally on a single Apple
Silicon Mac with 16 GB unified memory, using Metal acceleration
through Ollama (llama.cpp backend); no GPU cluster or cloud inference
is used.  Wall-clock latency is recorded per item as a secondary cost
metric alongside generated token counts.
...
of model scale at this size class: on GSM8K the few-shot-CoT gain
over direct answering is of the same order as the gain from scaling
the model from 1B to 3B (Section~\ref{sec:results-main}).  Second,
the effect is task-dependent: trace- and ensemble-based techniques
deliver their largest gains on GSM8K and are roughly neutral on
HumanEval.
...
Direct answering & 11.0 & -- & -- & -- & -- & -- \\
Zero-shot CoT & -- & -- & -- & -- & -- & -- \\
Few-shot CoT & 8.0 & -- & -- & -- & -- & -- \\
Self-consistency & -- & -- & -- & -- & -- & -- \\
%END-TABLE
\bottomrule
\multicolumn{7}{p{\dimexpr\textwidth-12pt\relax}}{$^{\mathrm{a}}$95\% paired-bootstrap CIs over items: $\pm 10$ points (GSM8K, $n = 100$) and $\pm 8$ points (HumanEval, $n = 164$) at accuracies near 0.5.}
\end{tabular}
\label{tab:results}
\end{table*}
...
On GSM8K, few-shot CoT raises accuracy over direct answering at every
scale, and the gain is comparable to the improvement from scaling the
model across the evaluated range (1B to 3B); self-consistency adds a
further gain
on top of few-shot CoT at a fourfold token cost
(Table~\ref{tab:results}, Table~\ref{tab:cost}).  On HumanEval the
ordering is compressed: no technique improves pass@1 by more than a
few points, and zero-shot CoT is at best neutral.  Differences from
direct answering are evaluated against the bootstrap intervals in the
table note, which are wide at $n = 100$; only differences exceeding
the interval width are treated as evidence of an effect.
...
declines as temperature rises, whereas self-consistency requires
$T > 0$ and is evaluated at its defined setting $T = 0.7$; the
ordering of the two curves, and whether they cross, is the empirical
content of H2.  Because the sweep is run on all three models, the
table also shows whether the interaction is stable across scale.
...
($p = 0.9$); self-consistency requires $T > 0$ and is evaluated at
its defined setting $T = 0.7$, while single-sample accuracy declines
with temperature.  (b) GSM8K accuracy by technique and model scale at
the reference decoding cell; the gain from direct answering to
few-shot CoT is comparable to the gain from the 1B to the 3B model.}
\label{fig:results}
\end{figure*}
...
on the same Metal backend.  Accuracy per thousand generated tokens is
...
Direct answering & 36 & 0.8 & -- & -- & -- & -- \\
Zero-shot CoT & -- & -- & -- & -- & -- & -- \\
Few-shot CoT & 182 & 6.2 & -- & -- & -- & -- \\
Self-consistency & -- & -- & -- & -- & -- & -- \\
%END-TABLE
\bottomrule
\end{tabular}
...
of a reported gain is a property of the particular sentence.  The
lexical factor is evaluated with four paraphrases on GSM8K and two on
HumanEval, so the HumanEval estimates are the noisier of the two.
...
3B & GSM8K & -- & -- & -- \\
3B & HumanEval & -- & -- & -- \\
Qwen2.5-Coder-3B & GSM8K & -- & -- & -- \\
Qwen2.5-Coder-3B & HumanEval & -- & -- & -- \\
%END-TABLE
\bottomrule
\end{tabular}
\label{tab:aer}
...
models run under identical Metal-accelerated decoding on a single
...
On GSM8K, few-shot CoT emits several times as many tokens as direct
answering (Table~\ref{tab:cost}), and self-consistency multiplies
...
single-sample accuracy degrades as temperature rises, while
sampling-based techniques such as self-consistency require a nonzero
temperature to be defined at all.
...
paraphrases give coarse lexical variance estimates.  The Ollama
...
may reject correct but slow solutions, and the 16 GB unified-memory
hardware limits the model scale that can be evaluated.  The
...
16 GB Apple Silicon Mac, which makes the protocol available to
researchers without GPU clusters.  We recommend that future studies
