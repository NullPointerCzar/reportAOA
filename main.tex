...
effect ratio is near one (0.82--1.00), so the technique gains largely
survive rewording of the prompt; on HumanEval it is mixed
(0.25--0.98) and degenerate where the technique effects vanish.
Temperature
...
single-sample accuracy declines with temperature at the smaller
scales and is roughly flat at the 3B scale.  We conclude with an
...
\begin{itemize}
  ...
  \item A controlled empirical study that isolates the algorithmic
        ...
        $(T, k, p)$, and vice versa, at a scale that runs entirely on
...
the design and is fixed before analysis: the seed is logged and the
complete item-ID lists are released with the reproducibility artefacts
at \url{https://github.com/sarwagya/prompt-algorithmic-effects}, so the
items are publicly identifiable and any run can reconstruct the same
set.  These sizes balance power against cost: at accuracies near 0.5,
...
classes of Section~\ref{sec:taxonomy}.
...
protocol of the original benchmark.
...
evaluation logs, and analysis scripts are released at
\url{https://github.com/sarwagya/prompt-algorithmic-effects}; the
harness is resumable, every record carries its cell and item
identifiers, and random seeds for every model call are recorded in the
output logs.
...
two tasks.  Two findings organise the results.  First, the
...
HumanEval is pass@1 on all 164 problems.  The model-scale comparison
appears in Fig.~\ref{fig:results}b.}
\centering
\footnotesize
...
Self-consistency & 11.0 & 20.1 & 88.0 & 41.5 & 81.0 & 75.0 \\
%END-TABLE
\bottomrule
...
(53\% to 70\%) and the coder model (6\% to 71\%) but hurts the 1B
model (12\% to 8\%); self-consistency adds a further gain over
few-shot CoT at the 3B (70\% to 88\%) and coder (71\% to 81\%) scales
at a fourfold token cost (Tables~\ref{tab:results} and~\ref{tab:cost}).
...
weakest configuration at every scale (Table~\ref{tab:results}).
...
declines with temperature at the 1B (8\% to 6\%) and coder (71\% to
62\%) scales and is roughly flat, with a mild peak at $T = 0.3$, at
the 3B scale (70\% to 66\%); self-consistency requires $T > 0$ and is
...
every scale---is the empirical content of H2.  Because the sweep is
...
0 & 8.0 & --- & 70.0 & --- & 71.0 & --- \\
0.3 & 4.0 & --- & 74.0 & --- & 67.0 & --- \\
0.7 & 6.0 & 11.0 & 72.0 & 88.0 & 69.0 & 81.0 \\
1 & 6.0 & --- & 66.0 & --- & 62.0 & --- \\
%END-TABLE
\bottomrule
\end{tabular}
...
($p = 0.9$); square markers at $T = 0.7$ denote self-consistency,
which requires $T > 0$ and is evaluated at its defined setting
$T = 0.7$, while single-sample accuracy declines
with temperature at the 1B and coder scales and is roughly flat at
the 3B scale.  (b) GSM8K accuracy by technique and model scale at the
...
output is checkable by execution (code).  Robustness to rewording is
high when the AER is near one---the algorithmic structure, not the
...
single-sample accuracy declines with temperature at the smaller
scales, while sampling-based techniques such as self-consistency
